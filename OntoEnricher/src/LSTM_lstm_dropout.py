...
class RelationPredictor(nn.Module):
    ...
    def __init__(self, emb_vals):
        ...
        self.lstm = nn.LSTM(self.input_dim, HIDDEN_DIM, NUM_LAYERS, bidirectional=bidirectional, batch_first=True, dropout=dropout)
    ...
    def forward(self, nodes, paths, counts, edgecounts, max_paths, max_edges):
        ...
        '''
        word_embed = self.name_embeddings(paths[:,:,:,0])
        pos_embed = self.pos_embeddings(paths[:,:,:,1])
        dep_embed = self.dep_embeddings(paths[:,:,:,2])
        dir_embed = self.dir_embeddings(paths[:,:,:,3])
        paths_embed = torch.cat((word_embed, pos_embed, dep_embed, dir_embed), dim=-1)
        nodes_embed = self.name_embeddings(nodes).reshape(-1, 2*self.EMBEDDING_DIM)
