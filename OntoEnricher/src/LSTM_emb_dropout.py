...
class RelationPredictor(nn.Module):
    ...
    def forward(self, nodes, paths, counts, edgecounts, max_paths, max_edges):
        ...
        '''
        word_embed = F.dropout(self.name_embeddings(paths[:,:,:,0]), p=dropout)
        pos_embed = F.dropout(self.pos_embeddings(paths[:,:,:,1]), p=dropout)
        dep_embed = F.dropout(self.dep_embeddings(paths[:,:,:,2]), p=dropout)
        dir_embed = F.dropout(self.dir_embeddings(paths[:,:,:,3]), p=dropout)
        paths_embed = torch.cat((word_embed, pos_embed, dep_embed, dir_embed), dim=-1)
        nodes_embed = F.dropout(self.name_embeddings(nodes), p=dropout).reshape(-1, 2*self.EMBEDDING_DIM)
