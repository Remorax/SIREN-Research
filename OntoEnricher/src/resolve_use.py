...
def closest_word_USE(argument):
    ...
    for (word, embed) in argument:
        ...
        sys.stdout.flush()
        results.append((word1, closest_word, max_sim))
    return results
...
def run():
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=10) as executor:
        for results in executor.map(closest_word_USE, zip(failed, failed_embeddings), chunksize=int(len(failed)/100)):
            for res in results:
