...
$("#pitfalls-div").on("click", ".add-cond", function(event){
    var row_id = "buttons-div-" + $(this).attr('id').split("-").slice(-1)[0];
    var condition_id = $("#" + row_id).prev().find("select").attr('id').split("-").slice(-1)[0];
    condition_id++;
    ...
    <div class='col'>\
        <div class='form-group' id='criticality-div-"+pitfall_count+"-"+condition_id+"'>\
            <select class='form-control criticality-select' name='criticality-select-"+pitfall_count+"' id='criticality-select-"+pitfall_count+"-"+condition_id+"'>\
	...
	$("#" + row_id).before(new_cond);
	condition_id++;
	var header_conditions = ($("#header").find(".col").length-2)/2;
	if (condition_id > header_conditions){
		var new_header = "<div class='col'>Predicate " + condition_id + "</div>\
		<div class='col'>Object " + condition_id + "</div>\
		<div class='col'>Criticality " + condition_id + "</div>";
		$("#actions-header").before(new_header);
	}
...
$( "#add-pitfall" ).click(function() {
	...
	$("#pitfalls-div").append("<div class='row' id='row-" + pitfall_count + "'>\
            ...
            <div class='col'>\
                <div class='form-group' id='criticality-div-" + pitfall_count + "-0'>\
