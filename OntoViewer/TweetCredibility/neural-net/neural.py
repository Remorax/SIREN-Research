...
csv_file_train = open('Randombook_csv_train.csv', 'w')   # Open file in write mode for training data
wr_train = csv.writer(csv_file_train, quoting=csv.QUOTE_ALL)

csv_file_test = open('Randombook_csv_test.csv', 'w')      # Open file in write mode for test data
wr_test = csv.writer(csv_file_test, quoting=csv.QUOTE_ALL)

wr_train.writerow(['tweet','topic'])
wr_test.writerow(['tweet','topic'])               # Copying the first row of excel file which contain column name of each field




for rownum in range(df.shape[0]):
    if rownum<=(.7*df.shape[0]):
        wr_train.writerow(df.iloc[rownum])   # Distributing the whole data into train and test data
    else:
        wr_test.writerow(df.iloc[rownum])
...
for i in range( 0, train_data['tweet'].size ):
    train_data_string.append(train_data['tweet'][i])    # Appending train data

for i in range( 0, test_data['tweet'].size ):
...
number_of_classes=7                                                    # Number of topics (class) are stored in a variable
